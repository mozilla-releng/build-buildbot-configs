...
WIN64_IXS      = ['w64-ix-slave%02i' % x for x in range(16,30) + range(42,44) + range(75,100) if x not in [22,80,85,86]] + \
                 ['w64-ix-slave%03i' % x for x in range(100,158) if x not in range(110,131)]
WIN64_REV2     = ['w64-ix-slave%02i' % x for x in range(6,16)] + \
...
TRY_MAC64      = []
TRY_WIN32_IXS  = []
TRY_WIN64_IXS  = ['w64-ix-slave%02i' % x for x in range(40,75) if x not in [42,43]]
TRY_WIN64_REV2 = ['w64-ix-slave%02i' % x for x in [3] + range(30,40)] + \
