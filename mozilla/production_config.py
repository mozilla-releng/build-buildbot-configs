...
WIN32_IXS      = []
WIN64_IXS      = []
WIN64_REV2     = ['w64-ix-slave%02i' % x for x in range(98,100) if x not in [80,85,86]] + \
                 ['w64-ix-slave%03i' % x for x in range(100,110) + range(111,148) + range(152, 158)] + \
                 ['b-2008-ix-%04i' % x for x in range(1,18) + range(65,89) + range(90,113) + range(161,169)] + \
                 ['b-2008-sm-%04d' % x for x in range(33, 65)]
MOCK_DL120G7   = ['bld-centos6-hp-%03d' % x for x in  range(13,14) + range(15,20)] +\
                 ['b-linux64-hp-%04d' % x for x in range(25,30)]
...
MOCK_IX        = ['bld-linux64-ix-%03d' % x for x in range(34, 38)] + \
...
TRY_WIN64_REV2 = ['w64-ix-slave%02i' % x for x in range(38,41) + range(65,75)] + \
                 ['w64-ix-slave%03i' % x for x in [110] + range(159,171)] + \
                 ['b-2008-ix-%04i' % x for x in range(18, 65) + range(30, 42) + range(173,179)] + \
                 ['b-2008-sm-%04d' % x for x in range(1, 33)]
TRY_MOCK_DL120G7 = ['bld-centos6-hp-%03d' % x for x in [30] + range(30,33) + [35] + range(40,43)] + \
